      ...
      <div className="max-width-container py-14 px-4 mb-8">
        ...
        <div className={`grid mobile:grid-cols-1 ${columnClass[maxColumns] || "laptop:grid-cols-3"} gap-6`}>
          {options.map((el) => (
            <div
              key={el.id}
              className="lift-card w-full h-full p-8 bg-white dark:bg-slate-900 border border-blue-100/80 dark:border-slate-800 rounded-xl"
            ...
            >
              ...
              {el?.list?.length > 0 && (
                <ul className="list-disc pl-5 mb-5 space-y-1">
                  {el.list.map((item) => (
                    <li key={item.id} className="text-[16px] text-slate-600 dark:text-slate-400">{item.content}</li>
                  ))}
                </ul>
              )}
              <Link to="/contact">
